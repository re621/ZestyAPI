...
describe("Posts", () => {
    ...
    test("Fetch many posts (with a limit)", async () => {
        const result = await esix.Posts.find({ limit: 10 });
        expect(result.status.code).toBe(200);
        expect(result.data.length).toBe(10);
    });
    test("Fetch many posts (with a page)", async () => {
        const result = await esix.Posts.find({ page: 2 });
        expect(result.status.code).toBe(200);
        expect(result.data.length).toBe(75);
    });
    test("Fetch many posts (both a page and a limit)", async () => {
        const result = await esix.Posts.find({ limit: 10, page: 2 });
        expect(result.status.code).toBe(200);
        expect(result.data.length).toBe(10);
    });
    ...
    test("Fetch many posts (by IDs)", async () => {
        const result = await esix.Posts.getMany([12345, 12346, 12347]);
        expect(result.status.code).toBe(200);
    ...
    test("Fetch many random posts", async () => {
        const result = await esix.Posts.randomMany();
        expect(result.status.code).toBe(200);
        expect(result.data.length).toBe(75);
    });
    test("Fetch many random posts (with everything)", async () => {
        const result = await esix.Posts.randomMany({ tags: ["horse"], limit: 10, page: 2 });
        expect(result.status.code).toBe(200);
        expect(result.data.length).toBe(10);
    });
});
